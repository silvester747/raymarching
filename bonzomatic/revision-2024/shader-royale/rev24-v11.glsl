...
#define M_2PI (2 * M_PI)
#define FFT_FACTOR 60.
#define NUM_TILES 12

#define MOVE_MOUNTAINS
#define WITH_TILES
#define WITH_SNOWMEN
#define WITH_SNOWMEN_COLOR
...
float GetDist(vec3 p) {
  ...
  float d_s1 = snowman(p - vec3(-.8 * jump_w * sin(fGlobalTime), fft(0.2) - .7 + jump_h, 5. - .8 * jump_w * cos(fGlobalTime)), 1.);
  float d_s2 = snowman(p - vec3(.8 * jump_w * sin(fGlobalTime), fft(0.3) - .7 + jump_h, 5. + .8 * jump_w * cos(fGlobalTime)), 1.);
...
vec4 GetLight(vec3 p, vec4 background_color) {
  ...
  if (length(p.xy) > 1.95) {
    ...
    // Transparency
    col = mix(col, background_color.xyz, .3 - .3 * length(col));
    if (length(col) < .3) {
      // Glass look
      col = mix(col, vec3(.8, .898, 1.), .1);
    }
  } else {
 #ifdef WITH_SNOWMEN_COLOR
    // Internal objects
 ...
 #endif
    col = mix(col, vec3(1.) * dif, .3 - .7 * length(col));
  }
...
float moving_mountains(in float offset) {
#ifdef MOVE_MOUNTAINS
  float shift = fGlobalTime / 10.;
  float y = mod(shift + 2.5 + offset, 5.) - 2.5;
  return y;
#else
  return offset;
#endif
...
vec4 background(void)
{
  ...
  float shift = fGlobalTime / 20.;
  
  // Middle
  float m1 = sdEquilateralTriangle(p - vec2(moving_mountains(0.), -1.2), 1.4 + fft(0.25));
  
  // Left
  float ml1 = sdEquilateralTriangle(p - vec2(moving_mountains(-.5), -1.2), 1. + .75 * fft(0.75));
  float ml3 = sdEquilateralTriangle(p - vec2(moving_mountains(-1.1), -1.4), 1. - fft(0.20));
  
  // Right
  float mr1 = sdEquilateralTriangle(p - vec2(moving_mountains(.4), -.9), 1. + .75 * fft(0.50));
  float mr2 = sdEquilateralTriangle(p - vec2(moving_mountains(1.), -1.05), 1. - 0.25 * fft(0.60));
  float mr3 = sdEquilateralTriangle(p - vec2(moving_mountains(1.4), -.8), 1. + 0.6 * fft(0.4));
  
  // Lonely mountain
  float m2 = sdEquilateralTriangle(p - vec2(moving_mountains(2.5), -1.2), 1. + 0.6 * fft(0.3));
