...
float tiles(vec3 p, int n)
{
  ...
  int fixed_id = int(floor((an - mod(fGlobalTime, pipi))/sp) + float(n)/2);
...
float GetDist(vec3 p) {  
  ...
  float s1 = sdSphere(p - vec3(0., -.4, 5.), .4);
  float s2 = sdSphere(p - vec3(0., .1, 5.), .3);
  float s3 = sdSphere(p - vec3(0., 0.5, 5.), .2);
  float s = opSmoothUnion(s1, s2, 0.03);
  s = opSmoothUnion(s, s3, 0.03);
  
  return min(d_tiles, s);
}
...
vec4 GetLight(vec3 p, vec4 background_color) {
  float pipi = 6.283185;
  float sp = pipi/float(NUM_TILES);
  float an = atan(p.y,p.x);
  int id = int(floor((an - mod(fGlobalTime, pipi))/sp));
  float soft_id = (an - mod(fGlobalTime, pipi))/sp;
  ...
  vec3 col = vec3(0.);
  vec3 col1 = vec3(1., 0., 0.) * smoothstep(.15, .6, fft(0.1)) * dif;
  vec3 col2 = vec3(0., 1., 0.) * smoothstep(.05, .6, fft(.4)) * dif;
  vec3 col3 = vec3(0., 0., 1.) * smoothstep(.0, .3, fft(.5)) * dif;
...
vec4 background(void)
{
  ...
  // Middle
  float m1 = sdEquilateralTriangle(p - vec2(0., -1.), 1.4 + fft(0.25));
  
  // Left
  float ml1 = sdEquilateralTriangle(p - vec2(-.5, -1.), 1. + .75 * fft(0.75));
  float ml3 = sdEquilateralTriangle(p - vec2(-1.1, -1.2), 1. - fft(0.20));
  
  // Right
  float mr1 = sdEquilateralTriangle(p - vec2(.4, -.8), 1. + .75 * fft(0.50));
